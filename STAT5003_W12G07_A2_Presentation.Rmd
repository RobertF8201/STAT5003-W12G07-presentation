...
<div style="flex:0 0 48%; font-size:14px;">

**Limitations**

- **Random undersampling discards ~90,000 majority-class rows** - introduces variance and a distribution shift relative to the true operating environment
- **XGBoost underperformed unexpectedly** (Macro-F1 ~0.27) - likely due to one-hot expansion of high-cardinality categoricals interacting poorly with the chosen weight scheme; needs revisiting
- **Naive Bayes' conditional independence assumption is violated** (e.g., speed and urbanisation are highly correlated)
- **No temporal holdout in final report run** - 2024 data prepared but not yet evaluated; potential temporal drift unmeasured
- Free-text fields (Street, Town, Identifying feature) were dropped - could carry signal
...
- **Wider mtry / ntree search** for RF - OOB curve had not converged at ntree = 100, so further gains likely
- **Target encoding** for high-cardinality categoricals (LGA, TU type) instead of one-hot - especially for XGBoost
- **Cost-sensitive learning** with explicit asymmetric misclassification costs (cost of missing a Fatal >> cost of false alarm)
- **Calibrated probabilities + threshold tuning** rather than argmax classification - lets stakeholders pick their own operating point
- **Geospatial features** from Lat/Lon (proximity to intersections, road class, road geometry)
- **Evaluate on 2024 temporal holdout** to quantify drift before deployment
