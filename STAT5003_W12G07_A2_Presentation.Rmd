...
<li>**Split** - 2024 data sealed as temporal holdout; 90/10 stratified split on 2020-2023; 5-fold CV</li>
</ol>
...
<tbody>
  ...
  <tr><td style="padding:4px 10px;">SMOTE (LR, NB)</td><td style="padding:4px 10px;">Synthetic oversampling applied inside training fold only; Fatal raised to ~10% - test set unmodified</td></tr>
...
<p style="font-size:10.5px; text-align:left; margin-top:4px; line-height:1.3; background:#f9f9f9; padding:5px; width:75%;"><b>Top predictors:</b> Street lighting, Month of crash, LGA, Key TU type, First impact type - structural & environmental factors dominate over temporal ones.</p>
</div>
...
<b>Top predictors by Gain:</b> Street lighting (Unknown/not stated dummy) dominates with 0.30 gain — nearly 5× the next predictor. Distance, pedestrian/motorcycle involvement, Latitude/Longitude, and Speed limit (100 km/h) follow. Variable names reflect one-hot expanded columns.
</p>
</div>
...
<div style="background:#fce4e4; border-left:3px solid #d62728; padding:7px 12px; flex:1;">
<b>⚠ Performance Note:</b> Despite an extensive grid search over nrounds (100–500), max_depth (3–6), and eta (0.03–0.3), XGBoost produced near-trivial predictions (Kappa ≈ 0.003). The tuning curve shows all configurations converging to a narrow 0.29–0.31 Macro-F1 band with no meaningful differentiation — suggesting the model's sequential-boosting structure did not benefit from inverse class weights as intended. See Model Evaluation for a full discussion.
</div>
</div>
...
<!-- LEFT SIDE -->
<div style="flex:0 0 44%; font-size:15px;">

<p style="font-size:16px; font-weight:bold; margin-bottom:6px; color:#1F4E79;">Final Model Performance: 2024 Temporal Holdout</p>

<p style="font-size:13px; margin-bottom:8px; line-height:1.5;">
The 2024 crash records were entirely excluded from training and tuning and used solely as an unseen <b>temporal holdout</b> — simulating real deployment on future data. The holdout contains <b>~35,000 records</b> at the natural class distribution (~1.4% Fatal), versus the balanced test set of ~13,500 records with ~6% Fatal used during model selection.
</p>

<table style="font-size:14px; width:100%; border-collapse:collapse; margin-bottom:10px;">
<thead>
<tr style="background:#2E74B5;">
  <th style="color:white; padding:5px 8px; text-align:left;">Metric</th>
  <th style="color:white; padding:5px 8px; text-align:center;">Test Set</th>
  <th style="color:white; padding:5px 8px; text-align:center;">2024 Holdout</th>
</tr>
</thead>
<tbody>
<tr style="background:#f2f2f2;">
  <td style="padding:5px 8px;">Macro-F1</td>
  <td style="padding:5px 8px; text-align:center;">0.7079</td>
  <td style="padding:5px 8px; text-align:center;">0.334</td>
</tr>
<tr>
  <td style="padding:5px 8px; color:#d62728; font-weight:bold;">Fatal Recall</td>
  <td style="padding:5px 8px; text-align:center; color:#d62728; font-weight:bold;">0.5106</td>
  <td style="padding:5px 8px; text-align:center; color:#d62728; font-weight:bold;">0.106</td>
</tr>
<tr style="background:#f2f2f2;">
  <td style="padding:5px 8px;">Weighted F1</td>
  <td style="padding:5px 8px; text-align:center;">0.7817</td>
  <td style="padding:5px 8px; text-align:center;">0.572</td>
</tr>
<tr>
  <td style="padding:5px 8px;">Kappa</td>
  <td style="padding:5px 8px; text-align:center;">0.5473</td>
  <td style="padding:5px 8px; text-align:center;">0.037</td>
</tr>
</tbody>
</table>

<p style="font-size:16px; font-weight:bold; margin-bottom:6px; color:#1F4E79;">Reflection</p>

<p style="font-size:13px; line-height:1.5;">
The sharp drop in Fatal Recall (0.51 → 0.106) reflects two compounding factors. First, <b>class-proportion mismatch</b>: the model was calibrated on a balanced training set where Fatal was ~6% of rows; at inference time in 2024, fatals are back to ~1.4% — a 4× rarer signal. Second, <b>temporal drift</b>: crash patterns, reporting conventions, or infrastructure conditions in 2024 may differ from the 2020–2023 training window in ways the model cannot anticipate. Together, these highlight that strong held-out validation metrics do not guarantee robustness under real deployment, particularly for rare, high-consequence events.
</p>
